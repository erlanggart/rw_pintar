...
export default function AktivitasMatiFormPage() {
  const { user } = useAuth();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await api.post('/aktivitas.php', { ...form, jenis_aktivitas: 'Mati' });
      toast.success('Data kematian berhasil dicatat');
      navigate('/aktivitas/mati');
    } catch (err) {
      toast.error(err.response?.data?.error || 'Gagal mencatat kematian');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="app-page">
      <AppPageHeader
        ...
        description="Halaman ini khusus untuk pencatatan kematian agar proses input lebih fokus dan tidak tercampur dengan daftar data."
        actions={(
          <Link to="/aktivitas/mati" className="app-button-secondary">
            <FiArrowLeft size={18} />
            Kembali ke Daftar
          </Link>
        )}
      ...
      <section className="app-panel">
        <div className="app-panel-header">
          <div>
            <h3 className="app-panel-title">Form Pencatatan Kematian</h3>
            <p className="app-panel-description">Pilih penduduk yang meninggal dan lengkapi data terkait.</p>
          </div>
          <span className="app-chip">Kematian</span>
        </div>
        <div className="app-panel-body">
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
              ...
              <div>
                <label className="app-label">Tanggal Meninggal *</label>
            ...
            <div className="rounded-xl border border-rose-100 bg-rose-50/80 p-4">
              ...
              <input type="text" value={form.penyebab_kematian} onChange={(e) => setField('penyebab_kematian', toUppercase(e.target.value))} className="app-input uppercase" />
            </div>
            ...
            <div className="app-inline-actions">
              ...
              <Link to="/aktivitas/mati" className="app-button-secondary">Batal</Link>
            </div>
          </form>
        </div>
      </section>
