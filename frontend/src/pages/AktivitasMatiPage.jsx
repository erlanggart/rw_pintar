...
export default function AktivitasMatiPage() {
  const { user } = useAuth();
  ...
  const handleDelete = async (id) => {
    if (!confirm('Yakin ingin menghapus data kematian ini?')) return;
  ...
  const stats = [
    { kicker: 'Total Kematian', value: data.length },
  ...
  const emptyMessage = hasWilayahFilter ? 'Tidak ada data kematian pada filter wilayah ini.' : 'Belum ada data kematian.';
  const summaryItems = [
  ...
  return (
    <div className="app-page">
      <AppPageHeader
        eyebrow="Aktivitas — Kematian"
        title="Catatan Kematian"
        description="Daftar pencatatan kematian warga di wilayah Anda. Input data baru dilakukan pada halaman form terpisah agar operator lebih fokus."
        stats={stats}
        actions={(
          <Link to="/aktivitas/mati/tambah" className="app-button-primary">
            <FiPlus size={18} />
            Catat Kematian
          </Link>
        )}
      ...
      <section className="app-panel">
        <div className="app-panel-header">
          <div>
            <h3 className="app-panel-title">Daftar Kematian</h3>
            <p className="app-panel-description">Catatan kematian warga yang sudah tersimpan.</p>
          </div>
            ...
            <div key={item.id} className="rounded-xl border border-slate-200/80 bg-slate-50/80 p-4 shadow-sm">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <p className="mt-1 font-mono text-xs text-slate-500">{item.nik || '-'}</p>
                </div>
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                <span className="app-badge app-badge-neutral">{item.tanggal_aktivitas}</span>
                <span className="app-badge app-badge-rose">KK {item.no_kk || '-'}</span>
              </div>

              <div className="mt-3 space-y-2 text-sm leading-6 text-slate-600">
                <p><span className="font-semibold text-slate-800">Penyebab:</span> <span className="uppercase">{formatUppercaseValue(item.penyebab_kematian)}</span></p>
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr key={item.id}>
                    ...
                    <td className="font-mono text-xs text-slate-500">{item.nik || '-'}</td>
                    <td className="text-slate-700 uppercase">{formatUppercaseValue(item.penyebab_kematian)}</td>
