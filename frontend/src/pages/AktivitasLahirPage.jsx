...
export default function AktivitasLahirPage() {
  const { user } = useAuth();
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      toast.success('Data kelahiran berhasil dihapus');
      fetchData();
    } catch (err) {
  ...
  const stats = [
    { kicker: 'Total Kelahiran', value: data.length },
    { kicker: 'Laki-laki', value: data.filter((d) => d.jenis_kelamin_bayi === 'L').length },
    { kicker: 'Perempuan', value: data.filter((d) => d.jenis_kelamin_bayi === 'P').length },
  ];
  const hasWilayahFilter = Boolean(selectedDesaId || selectedRwId || selectedRtId);
  const emptyMessage = hasWilayahFilter ? 'Tidak ada data kelahiran pada filter wilayah ini.' : 'Belum ada data kelahiran.';
  const summaryItems = [
  ...
  return (
    <div className="app-page">
      <AppPageHeader
        eyebrow="Aktivitas — Kelahiran"
        title="Catatan Kelahiran"
        description="Daftar pencatatan kelahiran warga baru di wilayah Anda. Input data baru dilakukan pada halaman form terpisah agar operator tidak bingung."
        stats={stats}
        actions={(
          <Link to="/aktivitas/lahir/tambah" className="app-button-primary">
            <FiPlus size={18} />
            Catat Kelahiran
          </Link>
        )}
      ...
      <section className="app-panel">
        <div className="app-panel-header">
          <div>
            <h3 className="app-panel-title">Daftar Kelahiran</h3>
            <p className="app-panel-description">Catatan kelahiran warga yang sudah tersimpan.</p>
          </div>
            ...
            <div key={item.id} className="rounded-xl border border-slate-200/80 bg-slate-50/80 p-4 shadow-sm">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="text-base font-semibold text-slate-950 uppercase">{formatUppercaseValue(item.nama_bayi, 'NAMA BELUM DIISI')}</p>
                  <p className="mt-1 text-xs font-medium uppercase tracking-[0.2em] text-slate-500">{item.tanggal_aktivitas}</p>
                </div>
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                <span className={`app-badge ${item.jenis_kelamin_bayi === 'L' ? 'app-badge-sky' : 'app-badge-rose'}`}>
                  {item.jenis_kelamin_bayi === 'L' ? 'LAKI-LAKI' : item.jenis_kelamin_bayi === 'P' ? 'PEREMPUAN' : '-'}
                </span>
                <span className="app-badge app-badge-neutral">KK {item.no_kk || '-'}</span>
              </div>

              <p className="mt-3 text-sm leading-6 text-slate-600 uppercase">{formatUppercaseValue(item.keterangan, 'TANPA KETERANGAN.')}</p>
            </div>
          ))}
        </div>
        ...
        <div className="hidden md:block">
          <div className="app-table-scroll">
            <table className="app-table">
              ...
              <tbody>
                {loading ? (
                  <tr><td colSpan="7" className="app-empty-row">Memuat data...</td></tr>
                ) : data.length === 0 ? (
                  <tr><td colSpan="7" className="app-empty-row">{emptyMessage}</td></tr>
                ) : data.map((item, i) => (
                  <tr key={item.id}>
                    <td>{i + 1}</td>
                    <td>{item.tanggal_aktivitas}</td>
                    <td className="font-medium text-slate-900 uppercase">{formatUppercaseValue(item.nama_bayi)}</td>
                    <td>
                      <span className={`app-badge ${item.jenis_kelamin_bayi === 'L' ? 'app-badge-sky' : 'app-badge-rose'}`}>
                        {item.jenis_kelamin_bayi === 'L' ? 'LAKI-LAKI' : item.jenis_kelamin_bayi === 'P' ? 'PEREMPUAN' : '-'}
                      </span>
                    ...
                    <td className="max-w-[260px] leading-6 text-slate-600 uppercase">{formatUppercaseValue(item.keterangan)}</td>
                    <td>
                      <button type="button" onClick={() => handleDelete(item.id)} className="app-icon-button text-rose-600 hover:border-rose-200 hover:bg-rose-50 hover:text-rose-700" title="Hapus">
                        <FiTrash2 size={16} />
                      </button>
