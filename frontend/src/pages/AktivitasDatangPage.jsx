...
export default function AktivitasDatangPage() {
  const { user } = useAuth();
  ...
  const handleDelete = async (id) => {
    if (!confirm('Yakin ingin menghapus data kedatangan ini?')) return;

    try {
      await api.delete(`/aktivitas.php?id=${id}`);
      toast.success('Data kedatangan berhasil dihapus');
      fetchData();
    } catch (err) {
  ...
  const stats = [
    { kicker: 'Total Kedatangan', value: data.length },
    ...
    { kicker: 'Keluarga Terkait', value: new Set(data.map((d) => d.keluarga_id).filter(Boolean)).size },
  ];
  const hasWilayahFilter = Boolean(selectedDesaId || selectedRwId || selectedRtId);
  const emptyMessage = hasWilayahFilter ? 'Tidak ada data kedatangan pada filter wilayah ini.' : 'Belum ada data kedatangan.';
  const summaryItems = [
  ...
  return (
    <div className="app-page">
      <AppPageHeader
        eyebrow="Aktivitas — Kedatangan"
        title="Catatan Kedatangan"
        description="Daftar pencatatan warga baru yang datang ke wilayah Anda. Input data baru dilakukan pada halaman form terpisah agar operator lebih fokus."
        stats={stats}
        actions={(
          <Link to="/aktivitas/datang/tambah" className="app-button-primary">
            <FiPlus size={18} />
            Catat Kedatangan
          </Link>
        )}
      ...
      <section className="app-panel">
        <div className="app-panel-header">
          <div>
            <h3 className="app-panel-title">Daftar Kedatangan</h3>
            <p className="app-panel-description">Catatan kedatangan warga baru yang sudah tersimpan.</p>
          </div>
            ...
            <div key={item.id} className="rounded-xl border border-slate-200/80 bg-slate-50/80 p-4 shadow-sm">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="text-base font-semibold text-slate-950 uppercase">{formatUppercaseValue(item.kepala_keluarga, 'KELUARGA TIDAK DIKETAHUI')}</p>
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                <span className="app-badge app-badge-sky">{item.tanggal_aktivitas}</span>
              </div>

              <div className="mt-3 space-y-2 text-sm leading-6 text-slate-600">
                <p><span className="font-semibold text-slate-800">Alamat asal:</span> <span className="uppercase">{formatUppercaseValue(item.alamat_asal)}</span></p>
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr key={item.id}>
                    ...
                    <td className="font-medium text-slate-900 uppercase">{formatUppercaseValue(item.kepala_keluarga)}</td>
                    <td className="max-w-[260px] leading-6 text-slate-600 uppercase">{formatUppercaseValue(item.alamat_asal)}</td>
                    <td className="max-w-[240px] leading-6 text-slate-600 uppercase">{formatUppercaseValue(item.keterangan)}</td>
                    <td>
                      <button type="button" onClick={() => handleDelete(item.id)} className="app-icon-button text-rose-600 hover:border-rose-200 hover:bg-rose-50 hover:text-rose-700" title="Hapus">
                        <FiTrash2 size={16} />
                      </button>
